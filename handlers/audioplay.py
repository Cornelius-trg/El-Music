            ...
            [
                [
                    InlineKeyboardButton(
                        text="✨ ɢʀᴏᴜᴘ",
                        url=f"https://t.me/wildentourageni"),
                    InlineKeyboardButton(
                        text="🌻 ᴄʜᴀɴɴᴇʟ",
                        url=f"https://t.me/syeindream")
                ]
            ]
        )
    ...
    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            raise DurationLimitError(
                f"❌ Video tidak boleh lebih dari  {DURATION_LIMIT} minute(s) tidak bisa diplay!"
            )
        ...
        return await lel.edit_text("❗ kau tidak memberikan ku file audio atau link youtube untuk stream!")
