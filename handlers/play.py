...
def cb_admin_check(func: Callable) -> Callable:
    async def decorator(client, cb):
        ...
        if cb.from_user.id in admemes:
            ...
            await cb.answer("kamu tidak diizinkan melakukan itu!", show_alert=True)
            return
    return decorator                                                                       
...
async def playlist(client, message):
    ...
    msg = "**Sedang memutar** on {}".format(message.chat.title)
    msg += "\n• "+ now_playing
    msg += "\n• Atas Permintaan "+by
    temp.pop(0)
    if temp:
...
async def settings(client, message):
    ...
    if stats:
        ...
        await message.reply("**tolong Aktifkan Voice Chat Terlebih dahulu.**")
...
async def hfmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not message.chat.id in DISABLED_GROUPS:
            await lel.edit("**music player berhasil diaktifkan.**")
            return
        DISABLED_GROUPS.remove(message.chat.id)
        await lel.edit(
            f"✅ **music player sudah di aktifkan di grup ini.**\n\n💬 {message.chat.id}"
        )
        ...
        if message.chat.id in DISABLED_GROUPS:
            await lel.edit("**music player sudah di nonaktifkan.**")
            return
        DISABLED_GROUPS.append(message.chat.id)
        await lel.edit(
            f"✅ **music player berhasil di nonaktifkan di grup ini.**\n\n💬 {message.chat.id}"
        )
    else:
        await message.reply_text(
            "**saya hanya tau** `/musicplayer on` **dan** `/musicplayer off`"
...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        ...
        if not queue:
            await cb.message.edit("**Sedang Tidak Memutar ❗**")
        temp = []
        for t in queue:
            temp.append(t)
        now_playing = temp[0][0]
...
async def m_cb(b, cb):
    ...
    if type_ == "pause":
        ...
        if chet_id in callsmusic.pytgcalls.active_calls:
            ...
            callsmusic.pytgcalls.leave_group_call(chet_id)
            await cb.message.edit("⏹ **Music Distop!**")
        else:
            await cb.answer("Assistant tidak terhubung ke voice chat!", show_alert=True)
...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                if message.chat.title.startswith("Channel Music: "):
                    await lel.edit(
                        f"<b>tolong tambahkan {user.first_name} ke channel mu.</b>",
                    )
                    pass
                try:
                ...
                try:
                    ...
                    await lel.edit(
                        "<b>pembantu bergabung ke group chat mu</b>",
                    )
    ...
    try:
        ...
        await lel.edit(
            f"<i>{user.first_name} telah di banned, beritahu admin untuk unban @{ASSISTANT_NAME} secara manual.</i>"
        )
        return
    text_links=None
    ...
    if audio:
        ...
        try:
            ...
            await lel.edit(
                "😕 **Sory, saya tidak menemukan lagu yang anda minta**\n• tolong cek judul nya dengan benaratau gunakan inline mode."
            )
            print(str(e))
            return
        dlurl=url
        ...
        try:
          ...
          await lel.edit("**tolong berikan judul lagu untuk memulai !**")
        # veez project
        try:
            toxxt = "⚡ __Pilih Lagu Yang Akan Diputar:__\n\n"
            j = 0
            useer=user_name
            ...
            while j < 6:
                ...
                toxxt += f" └ ⚡ __Disponsori oleh {BOT_NAME} A.I__\n\n"
                j += 1            
            keyboard = InlineKeyboardMarkup(
                [
                    [
            ...
            await lel.edit("__tidak ada daftar lagu, tolong masukan judul lagu dengan benar...__")
            ...
            try:
                ...
                await lel.edit(
                    "**❌ lagu tidak ditemukan.** tolong berikan judul lagu yang benar."
                )
                print(str(e))
                return
            dlurl=url
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        try:
            ...
            message.reply("**Voice chat grup tidak aktif tolong nyalakan terlebih dahulu, tidak bisa memutar lagu.**")
            return
        await message.reply_photo(
            photo="final.png",
...
async def lol_cb(b, cb):
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        chat_id,
        photo="final.png",
        caption=f"💡 **Track ditambahkan ke antrisn**\n\n🏷 **Nama:** [{title[:45]}]({url})\n⏱ **Durasi:** `{duration}`\n🎧 **Permintaan dari:** {r_by.mention}\n" \
               +f"\n🔢 **Track Posisi:** » `{position}` «",
        reply_markup=keyboard,
        )
...
async def ytplay(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        "<b>❗ Jadikan Saya Admin Terlebih Dahulu Sebelum Menggunakan. me</b>",
                    )
                    return
                ...
                try:
                    await USER.join_chat(invitelink)
                    await USER.send_message(
                        message.chat.id, "🤖: Saya Bergabung Untuk Memutar Musik Di Obrolan Suara"
                    )
                    await lel.edit(
                        "<b>💡 Asisten userbot Berhasil bergabung ke chat</b>",
                    )
    ...
    try:
        ...
        await lel.edit(
            f"<i>{user.first_name} telah di banned di grup ini, beritahu admin untuk unban @{ASSISTANT_NAME} secara manual.</i>"
        )
        return
    await lel.edit("🔎 **Mencari lagu...**")
    user_id = message.from_user.id
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        try:
            ...
            message.reply("**❗ Maaf, tidak ada voice chat aktif disini, tolong nyalakan voice chat terlebih dahulu**")
            return
        await message.reply_photo(
            photo="final.png",
